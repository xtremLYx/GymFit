...
import { getQuickSwaps } from './lib/essentials.js'
...
import ExerciseFilterSheet from './components/ExerciseFilterSheet.jsx'
import YearReport from './components/YearReport.jsx'
import ExerciseCategoryPicker from './components/ExerciseCategoryPicker.jsx'
...
export function loadClassic6Plan() {
  const [chestTri, backBi, legsShoulders] = generateDeterministicPlan('classic6', S().equipment, S().excludedEquipment)
  update(st => {
    st.routines.push(chestTri, backBi, legsShoulders)
    st.week[1] = chestTri.id; st.week[4] = chestTri.id
    st.week[2] = backBi.id;   st.week[5] = backBi.id
    st.week[3] = legsShoulders.id; st.week[6] = legsShoulders.id
  })
  toast(t('🇮🇳 Classic 6-Day Split loaded — Mon/Thu Chest+Tri · Tue/Fri Back+Bi · Wed/Sat Legs+Shoulders'))
}
...
function ExerciseDetail({ ex, close }) {
  ...
  const isMachine = ex.eq && ex.eq !== 'body weight'
  const swaps = getQuickSwaps(ex.id, st.equipment, [...(st.excludedEquipment || []), isMachine ? ex.eq : ''])

  const handleExcludeMachine = () => {
    useStore.getState().excludeEquipment(ex.eq)
    const bestAlt = swaps[0]?.ex
    if (bestAlt) {
      toast(t('Excluded {0}. Showing best alternative: {1}', ex.eq, bestAlt.n))
      close()
      exerciseDetailSheet(bestAlt)
    } else {
      toast(t('Excluded {0} across routines and library.', ex.eq))
      close()
    }
  }
  ...
  return <>
    ...
    {isMachine && (
      ...
      >
        <span>🚫</span> {t("I don't have this machine ({0})", ex.eq)}
      </button>
    )}
    ...
    {swaps.length > 0 && (
      <div style={{ marginTop: 14, marginBottom: 8 }}>
        <h4 className="sec" style={{ marginBottom: 6 }}>{t('Best Alternatives')}</h4>
        <div className="list" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {swaps.slice(0, 3).map(({ ex: alt, meta }) => (
            <div
              key={alt.id}
              className="item"
              onClick={() => { close(); exerciseDetailSheet(alt) }}
              style={{ padding: '8px 10px', cursor: 'pointer', background: 'var(--surface-2)', borderRadius: 'var(--r)' }}
            >
              <Thumb ex={alt} style={{ width: 36, height: 36, borderRadius: 6 }} />
              <div className="grow" style={{ marginLeft: 8 }}>
                <div style={{ fontWeight: 600, fontSize: 14 }} className="capitalize">{alt.n}</div>
                <div style={{ fontSize: 11, color: 'var(--label-2)' }}>{alt.tg || alt.bp} · {alt.eq}</div>
              </div>
              {meta?.badge && <span className="badge-glass" style={{ fontSize: 11 }}>{meta.badge}</span>}
            </div>
          ))}
        </div>
      </div>
...
export const exercisePicker = (onPick, options = {}) => {
  let onInstantAdd = null
  let inRoutineIds = []
  let routine = null

  if (typeof options === 'function') {
    onInstantAdd = options
  } else if (typeof options === 'object' && options !== null) {
    onInstantAdd = options.onInstantAdd
    inRoutineIds = options.inRoutineIds || []
  ...
  return ui().openSheet(close => (
    <ExerciseCategoryPicker
      ...
      inRoutineIds={inRoutineIds}
      routine={routine}
      close={close}
      onCustomEx={q => customExSheet(null, onPick, q)}
    />
  ))
}
...
function ExConfig({ ex, existing, onSave, onDelete, close, routine }) {
  ...
  return <>
    ...
    {ex.desc && <div className="exnote">{ex.desc}</div>}
    {instrFor(ex).length > 0 && (
      <details style={{ margin: '8px 0 14px', background: 'var(--surface-2)', padding: '10px 14px', borderRadius: 'var(--r)' }}>
        <summary style={{ fontWeight: 600, color: 'var(--label)', fontSize: 13, cursor: 'pointer', outline: 'none' }}>
          📖 {t('Instructions & Form Tips')}
        </summary>
        <ol className="steps-list" style={{ marginTop: 8, paddingLeft: 18 }}>
          {instrFor(ex).map((s, i) => <li key={i} style={{ fontSize: 13, color: 'var(--label-2)', marginBottom: 4 }}>{s}</li>)}
        </ol>
      </details>
    )}
