...
import { loadStarterPlan, loadClassic6Plan, confirmSheet, importFromApp } from '../sheets.jsx'
...
export default function Settings() {
  ...
  const { update, replaceState, setUser, pullState, pushState, signOut, signOutAll, resetDemo, restoreEquipment } = useStore()
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="lock" iconTint="var(--acc)" title={t('Guest Mode')} subtitle={t('All data is saved locally on this device — no account needed.')} />
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host openGym')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
          onClick={() => window.open(REPO, '_blank', 'noopener')} />
      </> : user ? <>
    ...
    {/* ---------- training & equipment profile ---------- */}
    <Section title={t('Training & Equipment')}>
      <Row
        icon="dumbbell"
        iconTint="var(--acc)"
        title={t('Environment')}
        subtitle={S.environment === 'gym' ? t('Commercial Gym') : S.environment === 'home' ? t('Home / Free Weights') : t('Bodyweight')}
      />
      {S.excludedEquipment && S.excludedEquipment.length > 0 ? (
        <>
          {S.excludedEquipment.map(eq => (
            <Row
              key={eq}
              icon="xmark"
              iconTint="var(--red)"
              title={t('Absent: {0}', eq)}
              subtitle={t('Filtered out from exercises & plans')}
            >
              <Button size="sm" variant="tinted" onClick={() => { restoreEquipment(eq); toast(t('Restored {0}', eq)) }}>
                {t('Restore')}
              </Button>
            </Row>
          ))}
          <Row
            icon="reset"
            iconTint="var(--blue)"
            title={t('Restore all absent machines')}
            accessory="chevron"
            onClick={() => {
              update(s => {
                s.equipment = [...new Set([...(s.equipment || []), ...(s.excludedEquipment || [])])]
                s.excludedEquipment = []
              })
              toast(t('All machines restored!'))
            }}
          />
        </>
      ) : (
        <Row
          icon="sparkles"
          iconTint="var(--teal)"
          title={t('No equipment excluded')}
          subtitle={t('All movements are currently visible.')}
        />
      )}
    </Section>
    ...
    <Section title={t('Data')}>
      ...
      <Row icon="sparkles" iconTint="var(--yellow)" title={t('Load Classic 6-Day Split (🇮🇳 Chest/Back/Legs)')} accessory="chevron" onClick={loadClassic6Plan} />
    ...
    {/* ---------- training & equipment ---------- */}
    <Section title={t('Training & Equipment')} footer={t('Sets your gym profile and powers smart exercise recommendations.')}>
      <Row icon="dumbbell" iconTint="var(--green)" title={t('Environment & Equipment')}
        subtitle={S.environment === 'gym' ? t('Commercial Gym ({0} items)', S.equipment?.length || 0) : S.environment === 'home' ? t('Home / Free Weights') : t('Bodyweight Only')}>
        <Button size="sm" variant="tinted" onClick={() => update(s => { s.onboardingDone = false })}>
          {t('Reconfigure')}
        </Button>
      </Row>
      <Row icon="sparkles" iconTint="var(--acc)" title={t('AI Workout Customizer')}
        subtitle={t('Intelligent preset adaptation & server assistance')} />
    </Section>
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install GymFit as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      GymFit · {t('free & open source (AGPL v3)')}<br />
