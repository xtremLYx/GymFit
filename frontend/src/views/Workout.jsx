...
import { effectiveRoutine, lastEntryFor, bestWeightFor, buildSets, setsDoneActive, supersetUnits, unitOf, setLabel, modeOf, EFFORT, effortOf, stepEffort, capEffort, defaultConfig } from '../lib/history.js'
...
import ExerciseSwapModal from '../components/ExerciseSwapModal.jsx'
import { getEssentialMeta } from '../lib/essentials.js'
...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onStartTimed, onSwap }) {
  ...
  const meta = getEssentialMeta(entry.id)
...
function ActiveWorkout() {
  ...
  const handleSwapConfirm = (newExId) => {
    if (swapTargetIndex == null) return
    const prevName = exOr(A.entries[swapTargetIndex].id).n
    const newEx = exOr(newExId)
    mutEntry(swapTargetIndex, e => {
      e.id = newExId
    })
    useUI.getState().toast(t('Swapped {0} for {1}', prevName, newEx.n))
  ...
  return <div className="narrow">
        ...
        <div className="ss-card">
          ...
          {unit.map((idx, k) => <div key={idx} className="ss-ex">
            ...
            <ExerciseBlock entryIdx={idx} compact
              onToggle={i => toggle(idx, i)} onField={(i, f, v) => setField(idx, i, f, v)} onAddSet={() => addSet(idx)} onRemoveSet={() => removeSet(idx)} onStartTimed={i => startTimed(idx, i)} onSwap={idx => setSwapTargetIndex(idx)} />
          </div>)}
        </div>
      ) : (
        <ExerciseBlock entryIdx={cur} onToggle={i => toggle(cur, i)} onField={(i, f, v) => setField(cur, i, f, v)} onAddSet={() => addSet(cur)} onRemoveSet={() => removeSet(cur)} onStartTimed={i => startTimed(cur, i)} onSwap={idx => setSwapTargetIndex(idx)} />
      )}
      ...
      onClick={() => exercisePicker(
        (ex, onSaved) => exConfigSheet(
          ...
          cfg => {
            ...
            if (onSaved) onSaved()
            useUI.getState().toast(t('“{0}” added to workout', ex.n))
          },
          null,
          S.routines.find(r => r.id === A.routineId)
        ),
        {
          onInstantAdd: (ex) => {
            const cfg = defaultConfig(ex.id)
            update(s => {
              const full = { ...cfg, id: ex.id }
              const plan = nextPrescription(s, full, s.routines.find(r => r.id === s.active.routineId))
              s.active.entries.push({ id: ex.id, target: { ...cfg }, plan, sets: applyPrescription(buildSets(s, full), plan) })
          ...
          inRoutineIds: A.entries.map(e => e.id),
          routine: S.routines.find(r => r.id === A.routineId)
        }
      )}
      icon="plus"
    >
    ...
    {swapTargetIndex !== null && (
      <div id="modal-root" className="open">
        <div className="mback" onClick={() => setSwapTargetIndex(null)} />
        <div className="sheet" style={{ maxHeight: '88vh' }}>
          <div className="grab" />
          <ExerciseSwapModal
            exerciseId={A.entries[swapTargetIndex].id}
            onSelect={handleSwapConfirm}
            close={() => setSwapTargetIndex(null)}
          />
        </div>
      </div>
    )}
