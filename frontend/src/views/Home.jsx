...
import { bwSheet, goalSheet, dayOverrideSheet, calendarSheet, startFlow, loadStarterPlan, loadClassic6Plan, bwDeltaColor } from '../sheets.jsx'
...
export default function Home() {
  ...
  const onToday = () => {
    ...
    else if (!S.routines.length) nav('/plan?builder=1')
    else dayOverrideSheet(todayISO())
  }

  return <div className="narrow">
    <div className="hdr">
      <div><h1>{user ? t('Hi {0}', user.name) : 'GymFit'}</h1><div className="sub">{today.toLocaleDateString(dateLocale(), { weekday: 'long', day: 'numeric', month: 'long' })}</div></div>
      ...
      <div className="today-row" onClick={onToday}>
        <div className="row" style={{ gap: 9, minWidth: 0 }}>
          <span className="lrow-i" style={{ background: S.active ? 'var(--orange)' : routine ? 'var(--acc)' : 'var(--surface-3)' }}>
            <Icon name={S.active ? 'timer' : routine ? glyphOf(routine.emoji) : (!S.routines.length ? 'plus' : 'moon')} />
          </span>
          <div style={{ minWidth: 0 }}>
            <div className="lbl2">{t('Today')}</div>
            <div className="ttl">{S.active ? t('{0} — in progress', S.active.name) : routine ? routine.name : (!S.routines.length ? t('No routines yet') : t('Rest day'))}{todayOvr && routine ? ' · ' + t('rescheduled') : ''}</div>
          </div>
        </div>
        {S.active ? <span className="tag" style={{ color: 'var(--orange)', background: 'color-mix(in srgb,var(--orange) 16%,transparent)' }}>{t('Resume')}</span>
          : routine ? <span className="tag acc">{t('Start')}</span>
          : !S.routines.length ? <span className="tag acc">{t('Create')}</span>
    ...
    {!S.routines.length && !S.active && (
      <div className="card" style={{ border: '1px solid color-mix(in srgb, var(--acc) 30%, transparent)' }}>
        <div className="row" style={{ gap: 10, marginBottom: 8 }}>
          <span className="lrow-i" style={{ background: 'var(--acc)' }}><Icon name="sparkles" /></span>
          <div className="big" style={{ fontSize: 20, fontWeight: 700 }}>{t('Ready to Train')}</div>
        </div>
        <div className="muted small" style={{ marginBottom: 14 }}>
          {t('You chose a clean custom split. Design your workouts with our interactive muscle heatmap, or create a quick routine.')}
        </div>
        <Button variant="primary" icon="plus" onClick={() => nav('/plan?builder=1')}>
