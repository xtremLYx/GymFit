...
export const DEF = {
  ...
  coach: null,
  // Apple Onboarding & Equipment Profile
...
export const useStore = create((set, get) => {
  ...
  return {
    ...
    excludeEquipment(eq) {
      get().update(s => {
        s.excludedEquipment = [...new Set([...(s.excludedEquipment || []), eq])]
        s.equipment = (s.equipment || []).filter(x => x !== eq)
      })
    },

    restoreEquipment(eq) {
      get().update(s => {
        s.excludedEquipment = (s.excludedEquipment || []).filter(x => x !== eq)
        if (!s.equipment.includes(eq)) {
          s.equipment.push(eq)
        }
      })
    },
